...
実装の正は [monitor/monitor.py](../../monitor/monitor.py)。ここにあった全文コピーは削除した。二重管理になり、実際に `RCON_PORT` の読み方が実装とずれていた（Terraform コードを載せていた旧 02 章を削除したのと同じ理由）。
...
`FILESYSTEM_NAME` は `02-post-setup.md` の手順8で確認した実測値を `terraform.tfvars` の `filesystem_name` に入れる。コードは書き換えない。
