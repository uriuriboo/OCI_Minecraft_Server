...
>
> PowerShell から WSL 経由で `bash -n` を呼ぶと、クォート処理の問題で**コマンドが実行されないまま成功したように見える**ことがある。実際にこれで CRLF の不具合を一度見逃した ([A1. 手順書との差分](A1-doc-reconciliation.md) の 3-6)。
>
...
| V-37 | `exposure_mode` の切り替え | [06-operations.md](06-operations.md) の手順 | ワールドを維持したまま切り替わる |
