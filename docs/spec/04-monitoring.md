...
既定は `false` で、この問題は起きない ([02-architecture.md](02-architecture.md) の「scale to zero を既定で無効にしている理由」)。有効にする場合は、死活判定を RCON ではなく mc-router (`mc-router:25565`) への status ping に変える改修が先に必要になる ([06-operations.md](06-operations.md) の将来の拡張候補)。
...
ウィジェット定義は [dashboards/oci-dashboard.json](../../dashboards/oci-dashboard.json)。OCID を埋めた貼り付け用の MQL は次で生成できる。
