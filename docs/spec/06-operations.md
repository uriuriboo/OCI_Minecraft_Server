...
ツールの導入は [A2. ツール導入](A2-toolchain.md) を参照。Terraform は tenv で、Python は uv で管理する。
...
| 半期 | バージョン更新の検討 | [07-tech-stack.md](07-tech-stack.md) |
...
`ansible.builtin.systemd_service` モジュールを使うため ansible-core に下限がある。値は [07. 構成技術・バージョン](07-tech-stack.md)、導入手順は [A2. ツール導入](A2-toolchain.md)。
...
| 無人時に mc を止めてメモリを空けたい | `mc_router_auto_scale = true`。ただし先に monitor.py の死活判定を RCON から `mc-router:25565` への status ping に変える必要がある ([04-monitoring.md](04-monitoring.md)) | mc-server で他のものを動かしたくなったら |
